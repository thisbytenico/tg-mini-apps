...
# 获取第一行的语言标识
header_row = sheet_buy[1]
language_identifiers = [cell.value for cell in header_row[1:]]  # 从第二列开始

# 获取第四行的替换内容
content_row = sheet_buy[4]
replace_contents = [cell.value for cell in content_row[1:]]  # 从第二列开始

# 创建语言标识到替换内容的映射
language_map = dict(zip(language_identifiers, replace_contents))
...
for filename in os.listdir('html2'):
    if filename.endswith('.html'):
        ...
        if lang_code in language_map:
            replace_content = language_map[lang_code]
        else:
